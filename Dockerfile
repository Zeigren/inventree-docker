...
RUN apt-get update && apt-get install -y --no-install-recommends git libmariadb-dev gcc libjpeg-dev zlib1g-dev make

RUN git clone --branch ${VERSION} --single-branch https://github.com/inventree/InvenTree.git ${INVENTREE_ROOT}

#uncomment to install new requirements in development
#COPY ./InvenTree/requirements.txt ${INVENTREE_ROOT}/requirements.txt

RUN pip install --no-cache-dir -U -r ${INVENTREE_ROOT}/requirements.txt mysqlclient gunicorn 
